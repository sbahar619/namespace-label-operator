...
var _ = Describe("NamespaceLabel Webhook", func() {
	...
	Describe("ValidateCreate", func() {
		...
		Context("When validating labels", func() {
			It("should allow valid Kubernetes labels", func() {
				fakeClient := fake.NewClientBuilder().WithScheme(scheme).Build()
				validator = &NamespaceLabelCustomValidator{Client: fakeClient}
			...
			It("should reject invalid label keys", func() {
				fakeClient := fake.NewClientBuilder().WithScheme(scheme).Build()
				validator = &NamespaceLabelCustomValidator{Client: fakeClient}
				...
				obj := &labelsv1alpha1.NamespaceLabel{
					...
					Spec: labelsv1alpha1.NamespaceLabelSpec{
						Labels: map[string]string{
							"": "value", // Empty label key is invalid
						},
					},
				}
		...
		Context("When validating protection patterns", func() {
			It("should allow valid glob patterns", func() {
				fakeClient := fake.NewClientBuilder().WithScheme(scheme).Build()
				validator = &NamespaceLabelCustomValidator{Client: fakeClient}
			...
			It("should reject invalid glob patterns", func() {
				fakeClient := fake.NewClientBuilder().WithScheme(scheme).Build()
				validator = &NamespaceLabelCustomValidator{Client: fakeClient}
				...
				obj := &labelsv1alpha1.NamespaceLabel{
					...
					Spec: labelsv1alpha1.NamespaceLabelSpec{
						Labels: map[string]string{"env": "test"},
						ProtectedLabelPatterns: []string{
							"[invalid-range", // Invalid glob pattern
						},
					},
				}
				...
				warnings, err := validator.ValidateCreate(ctx, obj)
				Expect(err).To(HaveOccurred())
				Expect(err.Error()).To(ContainSubstring("invalid protection pattern"))
				Expect(warnings).To(BeEmpty())
			})

			It("should reject empty protection patterns", func() {
				fakeClient := fake.NewClientBuilder().WithScheme(scheme).Build()
				validator = &NamespaceLabelCustomValidator{Client: fakeClient}
				...
				warnings, err := validator.ValidateCreate(ctx, obj)
				Expect(err).To(HaveOccurred())
				Expect(err.Error()).To(ContainSubstring("protection pattern cannot be empty"))
				Expect(warnings).To(BeEmpty())
			})
		})

		Context("When validating protection mode", func() {
			It("should allow valid protection modes", func() {
				fakeClient := fake.NewClientBuilder().WithScheme(scheme).Build()
				validator = &NamespaceLabelCustomValidator{Client: fakeClient}

				validModes := []labelsv1alpha1.ProtectionMode{
					labelsv1alpha1.ProtectionModeSkip,
					labelsv1alpha1.ProtectionModeWarn,
					labelsv1alpha1.ProtectionModeFail,
					"", // Empty is also valid (default)
				}
				...
				for _, mode := range validModes {
					...
					warnings, err := validator.ValidateCreate(ctx, obj)
					Expect(err).NotTo(HaveOccurred(), "Mode %s should be valid", mode)
					Expect(warnings).To(BeEmpty())
				}
			})

			It("should reject invalid protection modes", func() {
				fakeClient := fake.NewClientBuilder().WithScheme(scheme).Build()
				validator = &NamespaceLabelCustomValidator{Client: fakeClient}
				...
				obj := &labelsv1alpha1.NamespaceLabel{
					...
					Spec: labelsv1alpha1.NamespaceLabelSpec{
						Labels:         map[string]string{"env": "test"},
						ProtectionMode: "invalid-mode",
					},
				}

				warnings, err := validator.ValidateCreate(ctx, obj)
				Expect(err).To(HaveOccurred())
				Expect(err.Error()).To(ContainSubstring("invalid protection mode"))
				Expect(warnings).To(BeEmpty())
			})
		})
	})
